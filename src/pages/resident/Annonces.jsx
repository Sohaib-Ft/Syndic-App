...
export default function ResidentAnnonces() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold text-slate-800">{t.announcements}</h1><p className="text-slate-500 mt-1">{t.allAnnouncements}</p></div>
        ...
        {annonces.map((a) => (
          <div key={a.id} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
            <div className="flex items-start gap-3 mb-3">
              ...
              <span className="text-xs text-slate-400 mt-0.5">{new Date(a.createdAt).toLocaleDateString(dateLocale, { day: 'numeric', month: 'long', year: 'numeric' })}</span>
            </div>
