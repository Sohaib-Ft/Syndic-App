...
export default function SyndicDashboard() {
  ...
  if (!data) return <div className="text-center py-20 text-slate-500">{t.noDataAvailable}</div>;

  const kpis = [
    { label: t.apartments, value: data.kpis.totalAppartements, icon: Building2, color: 'from-blue-500 to-blue-600' },
    { label: t.activeResidents, value: data.kpis.residentsActifs, icon: Users, color: 'from-emerald-500 to-emerald-600' },
    { label: t.collectedThisMonth, value: `${data.kpis.totalEncaisseMois.toLocaleString(dateLocale)} DH`, icon: CreditCard, color: 'from-amber-500 to-orange-500' },
    { label: t.unpaidThisMonth, value: `${data.kpis.impayesMois.toLocaleString(dateLocale)} DH`, icon: AlertTriangle, color: 'from-red-500 to-rose-500' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
          <h3 className="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
            <CheckCircle className="w-5 h-5 text-emerald-500" /> {t.latestPayments}
          </h3>
          <div className="space-y-3">
            {data.derniersPaiements.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-8">{t.noPayment}</p>
              ...
              <div key={p.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{t.apt} {p.appartement.numero}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold text-emerald-600">{p.montant.toLocaleString(dateLocale)} DH</p>
                  <p className="text-xs text-slate-400">{p.datePaiement ? new Date(p.datePaiement).toLocaleDateString(dateLocale) : ''}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
          <h3 className="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
            <Clock className="w-5 h-5 text-red-500" /> {t.lateResidents}
          </h3>
          <div className="space-y-3">
            {data.residentsEnRetard.length === 0 ? (
              <div className="text-center py-8">
                <CheckCircle className="w-12 h-12 text-emerald-300 mx-auto mb-2" />
                <p className="text-sm text-slate-400">{t.allUpToDate}</p>
              </div>
              ...
              <div key={p.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-red-50/50 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-700">{p.resident.prenom} {p.resident.nom}</p>
                    <p className="text-xs text-slate-400">{t.months[p.mois]} {p.annee} - {t.apt} {p.appartement.numero}</p>
                  </div>
                </div>
                <span className="text-sm font-semibold text-red-500">{p.montant.toLocaleString(dateLocale)} DH</span>
              </div>
            ))}
          </div>
        </div>
